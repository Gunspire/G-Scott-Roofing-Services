...
import { COMPANY, waMeHref } from "../lib/company";

const FEEDBACK_EMAIL = COMPANY.email;
const PHONE_NUMBER_RAW = COMPANY.phoneRaw;
const PHONE_NUMBER_DISPLAY = COMPANY.phoneDisplay;

const DEFAULT_GOOGLE_REVIEW_URL = `https://www.google.com/search?q=${encodeURIComponent(
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-6 sm:px-10 py-10 sm:py-12">
          <div className="max-w-xl">
            <p className="text-sm font-semibold text-[#89713B]">{COMPANY.name}</p>
              ...
              {[1, 2, 3, 4, 5].map((v) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "p-2 rounded-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#89713B] focus-visible:ring-offset-2",
                      "hover:bg-slate-50"
                    )}
                  >
                    <Star
        ...
        <div className="px-6 sm:px-10 py-6 bg-slate-900 text-slate-200 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <a
            className="text-sm font-bold text-white hover:text-[#E7DDC9] transition-colors"
            href={waMeHref(
              COMPANY.whatsapp.phoneWaMe,
              `Hi ${COMPANY.name} — I’d like some help with my recent job.`
            )}
            target="_blank"
            rel="noreferrer"
          >
