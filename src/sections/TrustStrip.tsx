          ...
          <div className="flex animate-scroll gap-16 items-center whitespace-nowrap">
            {[...items, ...items].map((item, i) => (
              <div key={i} className="flex items-center gap-3 group cursor-default">
                <div className="w-12 h-12 rounded-full bg-[#89713B]/10 flex items-center justify-center group-hover:bg-[#89713B] transition-colors duration-300">
                  {"icon" in item ? (
                    <item.icon className="w-6 h-6 text-[#89713B] group-hover:text-white transition-colors duration-300" />
                  ) : (
                    <img
                      src={item.imageSrc}
                      alt={item.imageAlt}
